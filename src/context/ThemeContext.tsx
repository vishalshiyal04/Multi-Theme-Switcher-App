...
// ─── Font link manager (one <link> per URL, ref-counted) ──────────────────────
...
export const ThemeProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    // Body classes — only touch what we own
    ...
    // <meta name="color-scheme"> for native browser UI (scrollbars, inputs)
    ...
    // <html> dark class — for Tailwind's darkMode: 'class' strategy
    document.documentElement.classList.toggle('dark', isDark);
  }, [currentThemeName, theme, isDark]);

  // Sync with OS preference changes at runtime
  useEffect(() => {
